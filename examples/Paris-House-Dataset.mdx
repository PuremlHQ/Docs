...
```bash
pip install torch scikit-learn scikit-image numpy opencv-python  xgboost pandas pytorch_tabnet
```
...
```
<Note> {" "} You can fetch the dataset using pureml.dataset.fetch() {" "}</Note>
```python
