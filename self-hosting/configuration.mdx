...
| ----------- | ----------------------------------- | ------------------------ |
| BACKEND_URL | backend url env to connect frontend | http://backend:8080/api/ |
...
<Tabs>
  <Tab title="Windows">
    ...
    2. CLI promt to know if user is using self-hosting
    - If yes then enter `Y`
    - If no then enter `N`
  ...
  <Tab title="Mac/Linux">
    ...
    2. Run `pureml init` command to setup the puremlconfig.yml file.
    3. CLI promt to know if user is using self-hosting
    - If yes then enter `Y`
    - If no then enter `N`
    4. Give the self-hosted backend and frontend url.
    > If no changes are made to ports or domains (using localhost) then go ahead and click enter to use the default values
    5. Run below command to start
    <CodeGroup>
